...
public class FWorld extends World {
  ...
  class ConcreteContactListener implements ContactListener {
    public void add(ContactPoint point) {
      ...
      try {
        m_world.m_contactStartedMethod.invoke(hAPI_Fisica.parent(),
                                              new Object[] { contact });
      } catch (Exception e) {
    ...
    public void persist(ContactPoint point) {
      ...
      try {
        m_world.m_contactPersistedMethod.invoke(hAPI_Fisica.parent(),
                                                new Object[] { contact });
      } catch (Exception e) {
    ...
    public void remove(ContactPoint point) {
      ...
      try {
        m_world.m_contactEndedMethod.invoke(hAPI_Fisica.parent(),
                                            new Object[] { contact });
      } catch (Exception e) {
    ...
    public void result(ContactResult point) {
      ...
      try {
        m_world.m_contactResultMethod.invoke(hAPI_Fisica.parent(),
                                            new Object[] { result });
      } catch (Exception e) {
  ...
  public void mouseEvent(MouseEvent event){
    ...
    if (event.getAction() == event.PRESS
      ...
      grabBody(hAPI_Fisica.screenToWorld(event.getX()), hAPI_Fisica.screenToWorld(event.getY()));
    ...
    if (event.getAction()  == event.DRAG) {
      ...
      dragBody(hAPI_Fisica.screenToWorld(event.getX()), hAPI_Fisica.screenToWorld(event.getY()));
  ...
  public FWorld(float topLeftX, float topLeftY, float bottomRightX, float bottomRightY) {
    super(new AABB(new Vec2(topLeftX,
                                                 topLeftY),
                   new Vec2(bottomRightX,
                                                 bottomRightY)),
          hAPI_Fisica.screenToWorld(new Vec2(0.0f, 10.0f)),                  // gravity vertical downwards 10 m/s^2
    ...
    //hAPI_Fisica.parent().registerMouseEvent(this);

    hAPI_Fisica.parent().registerMethod("mouseEvent",this);
    
    // Get the contactStarted(), contactPersisted() and contactEnded()
    // methods from the sketch
    try {
      m_contactStartedMethod =
        hAPI_Fisica.parent().getClass().getMethod("contactStarted",
    ...
    try {
      m_contactPersistedMethod =
        hAPI_Fisica.parent().getClass().getMethod("contactPersisted",
                                             new Class[] { FContact.class });
    } catch (Exception e) {
      // no such method, or an error.. which is fine, just ignore
    }
    ...
    try {
      m_contactEndedMethod =
        hAPI_Fisica.parent().getClass().getMethod("contactEnded",
                                             new Class[] { FContact.class });
    } catch (Exception e) {
      // no such method, or an error.. which is fine, just ignore
    }
    ...
    try {
      m_contactResultMethod =
        hAPI_Fisica.parent().getClass().getMethod("contactResult",
  ...
  public FWorld() {
    this(hAPI_Fisica.screenToWorld(-hAPI_Fisica.parent().width), hAPI_Fisica.screenToWorld(-hAPI_Fisica.parent().height), hAPI_Fisica.screenToWorld(2*hAPI_Fisica.parent().width), hAPI_Fisica.screenToWorld(2*hAPI_Fisica.parent().height));
  }
  ...
  public void setGrabbable(boolean value) {
    ...
    if (m_grabbable) {
      ...
      //hAPI_Fisica.parent().unregisterMouseEvent(this);

      hAPI_Fisica.parent().unregisterMethod("mouseEvent",this);
    }
  }
  ...
  public void setEdges(float topLeftX, float topLeftY, float bottomRightX, float bottomRightY, int color) {
    ...
    float box_thickness = 3.0f; //cm

    left = new FBox(box_thickness, height);
    left.setStaticBody(true);
    left.setGrabbable(false);
    left.setFillColor(color);
    left.setStrokeColor(color);
    left.setPosition(topLeftX-box_thickness/4.0f, ymid);
    addBody(left);

    right = new FBox(box_thickness, height);
    right.setStaticBody(true);
    right.setGrabbable(false);
    right.setFillColor(color);
    right.setStrokeColor(color);
    right.setPosition(bottomRightX+box_thickness/4.0f, ymid);
    addBody(right);

    top = new FBox(width, box_thickness);
    top.setStaticBody(true);
    top.setGrabbable(false);
    top.setFillColor(color);
    top.setStrokeColor(color);
    top.setPosition(xmid, topLeftY-box_thickness/4.0f);
    addBody(top);

    bottom = new FBox(width, box_thickness);
    bottom.setStaticBody(true);
    bottom.setGrabbable(false);
    bottom.setFillColor(color);
    bottom.setStrokeColor(color);
    bottom.setPosition(xmid, bottomRightY+box_thickness/4.0f);
    addBody(bottom);
  ...
  public void setEdges(float topLeftX, float topLeftY, float bottomRightX, float bottomRightY) {
    setEdges(topLeftX, topLeftY, bottomRightX, bottomRightY, hAPI_Fisica.parent().color(0));
  }
  ...
  public void setEdges(PApplet applet, int color) {
    setEdges(0, 0, hAPI_Fisica.screenToWorld(applet.width), hAPI_Fisica.screenToWorld(applet.height), color);
  }
  ...
  public void setEdges(int color) {
    setEdges(hAPI_Fisica.parent(), color);
  }
  ...
  public void setEdges() {
    setEdges(hAPI_Fisica.parent(), hAPI_Fisica.parent().color(0));
  }
  ...
  public ArrayList getBodies( float x, float y, boolean getStatic, int count ) {
    // Make a small box.
    //Vec2 p = hAPI_Fisica.screenToWorld(x, y);
    Vec2 p = new Vec2(x,y); 
  ...
  public int raycast(float x1, float y1, float x2, float y2, FBody[] bodies, int maxCount, boolean solidShapes) {
    ...
    segment.p1.set(new Vec2(x1, y1));
    segment.p2.set(new Vec2(x2, y2));
