...
public abstract class FBody extends FDrawable {
  ...
  protected float m_gravityScale = 1.0f; 
  ...
  protected boolean h_isHaptic = true; 
  protected float h_damping = 0.0f; 
  protected float h_stiffness =0.0f; 
  ...
  protected void postDrawDebug(PGraphics applet) {
    ...
    if (getBox2dBody() != null) {
        ...
        AABB aabb = getAABB();
        Vec2 lower = hAPI_Fisica.worldToScreen(aabb.lowerBound);
        Vec2 upper = hAPI_Fisica.worldToScreen(aabb.upperBound);
        ...
        Vec2 cent = hAPI_Fisica.worldToScreen(getBox2dBody().getWorldCenter());
        ...
        infobox += "w: "; 
        if ( width <= 1.0 ) {
            infobox += df.format(width*10.0) + "mm";
        } else {
            infobox += df.format(width) + "cm";
        }
        infobox += "\n";
        
        infobox += "h: ";
        if ( height <= 1.0 ) {
            infobox += df.format(height*10.0) + "mm";
        } else {
            infobox += df.format(height) + "cm";
        }
        infobox += "\n";
  ...
  protected void applyMatrix(PGraphics applet){
    applet.translate(hAPI_Fisica.worldToScreen(getX()), hAPI_Fisica.worldToScreen(getY()));
  ...
  public void addForce( float fx, float fy ){
    ...
    m_force.x += hAPI_Fisica.screenToWorld(fx);
    m_force.y += hAPI_Fisica.screenToWorld(fy);
  }
  ...
  public void addImpulse( float fx, float fy ){
    ...
    // TODO: w2s (world 2 screen)
    if (m_body != null) {
      m_body.applyImpulse(new Vec2(fx, fy), m_body.getWorldCenter());
    }
  ...
  public void addForce( float fx, float fy, float px, float py ){
    // TODO: w2s (world 2 screen)
    if (m_body != null) {
      m_body.applyForce(new Vec2(fx, fy), m_body.getWorldCenter().add(new Vec2(px, py)));
    }
  ...
  public void addImpulse( float fx, float fy, float px, float py ){
    // TODO: w2s (world 2 screen)
    if (m_body != null) {
      m_body.applyImpulse(new Vec2(fx, fy), m_body.getWorldCenter().add(new Vec2(px, py)));
    }
   ...
   *
   * @return the horizontal velocity of the body in simulation world units per second
   */
   ...
   *
   * @return the vertical velocity of the body in simulation world units per second
   */
   ...
   *
   * @param vx  the horizontal velocity of the body in simulation world units per second
   * @param vy  the vertical velocity of the body in simulation world units per second
   */
   ...
   *
   * @param dvx  the horizontal velocity to be added to the body in simulation world units per second
   * @param dvy  the vertical velocity to be added to the body in simulation world units per second
   */
  public void adjustVelocity( float dvx, float dvy ){
    // TODO: w2s (world 2 screen)
    if (m_body != null) {
      m_body.setLinearVelocity(new Vec2(getVelocityX() + dvx, getVelocityY() + dvy) );
      m_body.wakeUp();
    }

    m_linearVelocity= new Vec2(getVelocityX() + dvx, getVelocityY() + dvy);
  }

  /**
   * Returns the horizontal position of the body.
   *
   * @return the horizontal position of the body in simulation world units
   * @see #getY
   ...
   *
   * @return the vertical position of the body in simulation world units
   * @see #getX
   * @see #setPosition(float,float)
   */
  public float getY(){
   ...
   *
   * @param x  the horizontal position of the body in simulation world units
   * @param y  the vertical position of the body in simulation world units
   */
   ...
   *
   * @param dx  the horizontal position to be added to the body in simulation world units
   * @param dy  the vertical position to be added to the body in simulation world units
   */
  public void adjustPosition( float dx, float dy ){
    // TODO: w2s (world 2 screen)
    if (m_body != null) {
      m_body.setXForm(new Vec2(getX() + dx, getY() + dy), m_body.getAngle());
    }

    m_position= new Vec2(getX() + dx, getY() + dy);
  }
   ...
   *
   * @return the haptic stiffness coefficient of the body in dyne per meter (CGS units)
   * @see #setHapticStiffness(float) 
   * @see #setHapticStiffness(hStiffness)
   */
  public float getHapticStiffness(){
   ...
   *
   * @return the haptic damping coefficient of the body in dyne seconds per 
   * @see #setHapticDamping(float)
   * @see #setHapticDamping(hDamping)
   */
  public float getHapticDamping(){
   ...
   *
   * @param value   if {@code true} the body will be a haptic body.  If  if {@code false} the body will not generate a force when colliding with other bodies
   */
  public void setHaptic( boolean value ){
    if(!value){
     h_stiffness = 0.0f; 
     h_damping = 0.0f; 
    }
   ...
   *
   * @param value   if {@code true} the body will be a haptic body.  If  if {@code false} the body will not generate a force when colliding with other bodies
   * @param stiffness   sets the stiffness of the haptic body for direct collision rendering 
   * @param damping   sets the damping coefficent of the haptic body for direct collision rendering 
   */
  public void setHaptic( boolean value, float stiffness, float damping ){
    if( m_body != null ) {
          setHapticStiffness(stiffness); 
          setHapticDamping(damping); 
      }
   ...
   * @return   if {@code true} the body is a haptic body.  Resultant forces will be calculated between this body and other bodies based on penalty based formulations. 
   */
   ...
   *
   * @param stiffness  The stiffness of the haptic body. Resultant forces are proportional to the amount of penetration between bodies 
   */
  public void setHapticStiffness( float stiffness ){
    if( m_body != null ) {
          h_stiffness = stiffness; 
      }
   ...
   *
   * @param damping  The damping of the haptic body. Resultant forces are proportional to the amount of penetration between bodies 
   */
  public void setHapticDamping( float damping ){
    if( m_body != null ) {
          h_damping = damping; 
      }
      h_damping = damping; 
  ...
  protected Vec2 getLocalWorldPoint(Vec2 p) {
    ...
    XForm xf = new XForm();
    xf.position.set(new Vec2(getX(), getY()));
