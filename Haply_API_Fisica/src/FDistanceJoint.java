...
public class FDistanceJoint extends FJoint {
  ...
  protected JointDef getJointDef(FWorld world) {
    ...
    md.length = hAPI_Fisica.screenToWorld(m_length);
  ...
  public void setLength(float length) {
    if ( m_joint != null ) {
      ((DistanceJoint)m_joint).m_length = (length);
    }
  ...
  public void setAnchor2(float x, float y) {
    if (m_joint != null) {
      ((DistanceJoint)m_joint).getAnchor2().set(x,y);
    }

    m_anchor2 = new Vec2(x, y);
  }
  ...
  public void setAnchor1(float x, float y) {
    if (m_joint != null) {
      ((DistanceJoint)m_joint).getAnchor1().set(x,y);
    }

    m_anchor1 = new Vec2(x, y);
  }
  ...
  public void draw(PGraphics applet){
    ...
    applet.ellipse(hAPI_Fisica.worldToScreen(getAnchor1X()), hAPI_Fisica.worldToScreen(getAnchor1Y()), 5, 5);
    applet.line(hAPI_Fisica.worldToScreen(getAnchor1X()), hAPI_Fisica.worldToScreen(getAnchor1Y()), hAPI_Fisica.worldToScreen(getAnchor2X()), hAPI_Fisica.worldToScreen(getAnchor2Y()));
    applet.ellipse(hAPI_Fisica.worldToScreen(getAnchor2X()), hAPI_Fisica.worldToScreen(getAnchor2Y()), 5, 5);
  ...
  public void drawDebug(PGraphics applet){
    ...
    if (numSpins <= 0) {
        ...
        applet.translate(hAPI_Fisica.worldToScreen(getAnchor1X()), hAPI_Fisica.worldToScreen(getAnchor1Y()));
        
        float ang = hAPI_Fisica.parent().atan2(hAPI_Fisica.worldToScreen(getAnchor2Y()-getAnchor1Y()), hAPI_Fisica.worldToScreen(getAnchor2X()-getAnchor1X()));
        float dist = hAPI_Fisica.parent().dist(hAPI_Fisica.worldToScreen(getAnchor1X()), hAPI_Fisica.worldToScreen(getAnchor1Y()), hAPI_Fisica.worldToScreen(getAnchor2X()), hAPI_Fisica.worldToScreen(getAnchor2Y()));
        applet.rotate(ang);
    
        if (m_length>0) {
            applet.rect(hAPI_Fisica.worldToScreen(dist/2), 0, hAPI_Fisica.worldToScreen(m_length), 12);
        }
        ...
        for (int i=0; i<numSpins; i++) {
            x = hAPI_Fisica.parent().map(i+1, 0, numSpins+1, 10, dist-10);
            y = ((i % 2)*2-1)*4;
            applet.vertex(hAPI_Fisica.worldToScreen(x), hAPI_Fisica.worldToScreen(y));
        }
        
        x = hAPI_Fisica.parent().map(numSpins+1, 0, numSpins+1, 10, dist-10);
        applet.vertex(hAPI_Fisica.worldToScreen(x), 0);
        applet.vertex(hAPI_Fisica.worldToScreen(dist), 0);
