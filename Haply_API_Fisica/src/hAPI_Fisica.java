...
/**
 * The hAPI_Fisica library maps between the graphic world (pixels) created in in the PApplet 
 *object to the simulation world cm. The simulation world is in CGS (centimeter, gram, seconds). 
 * Represents the library hAPI_Fisica.  It is mainly used to initialize it with the PApplet:
 * <pre>
 * {@code
 ...
 *
 */
public class hAPI_Fisica implements PConstants{
  ...
  /**
   * Exception thrown when the library has not been initialized.  The method {@link hAPI_Fisica#init(PApplet)} must be called before any use of the library.
   *
  ...
  public static class LibraryNotInitializedException extends NullPointerException{
    ...
    LibraryNotInitializedException(){
      super("Must call hAPI_Fisica.init(this); before using this library.");
    }
  }
  ...
  /**
   * Initialize the library.  Must be called before any use of the library.  Must be called by passing the PApplet.  e.g. {@code hAPI_Fisica.init(this)}
   *
   * @param applet  The applet on which to use the library.  This library can only be used with one applet
   */
