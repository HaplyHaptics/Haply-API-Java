...
public class FConstantVolumeJoint extends FJoint {
  ...
  public void draw(PGraphics applet){
    ...
    if (m_image != null ) {
      ...
      applet.translate(hAPI_Fisica.worldToScreen(c.x), hAPI_Fisica.worldToScreen(c.y));
      drawImage(applet);
      applet.popMatrix();
    } else {
      if (m_bodies.size()>0) {
        applet.beginShape();
        for (int i=0; i<m_bodies.size(); i++) {
          applet.vertex(hAPI_Fisica.worldToScreen(((FBody)m_bodies.get(i)).getX()), hAPI_Fisica.worldToScreen(((FBody)m_bodies.get(i)).getY()));
        }
  ...
  public void drawDebug(PGraphics applet){
    ...
    if (m_bodies.size()>0) {
      ...
      for (int i=0; i<m_bodies.size(); i++) {
        applet.ellipse(hAPI_Fisica.worldToScreen(((FBody)m_bodies.get(i)).getX()), hAPI_Fisica.worldToScreen(((FBody)m_bodies.get(i)).getY()), 5, 5);
      }
    }
