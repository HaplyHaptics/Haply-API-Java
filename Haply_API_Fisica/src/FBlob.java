...
public class FBlob extends FBody {
  ...
  protected float m_vertexSize = .15f;  // in world coords
  ...
  public float getVertexX(int i){
    return ((Vec2)m_vertices.get(i)).x;
  }
  ...
  public float getVertexY(int i){
    return ((Vec2)m_vertices.get(i)).y;
  }
  ...
  public void setAsCircle(float x, float y,
    ...
    for (int i=0; i<vertexCount; i++) {
      float angle = hAPI_Fisica.parent().map(i, 0, vertexCount, 0, hAPI_Fisica.parent().TWO_PI);
      float vx = x + size/2 * hAPI_Fisica.parent().sin(angle);
      float vy = y + size/2 * hAPI_Fisica.parent().cos(angle);
  ...
  public void setVertexSize(float size){
    m_vertexSize = hAPI_Fisica.screenToWorld(size);
  }
