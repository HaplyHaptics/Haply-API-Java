...
public class Device{
  ...
  */
	public void device_set_parameters(){
      ...
      motorParams[0] = 0;
		...
		byte[] encMtrSen = new byte[motorParams.length  + encoderParams.length + sensorParams.length];
		System.arraycopy(motorParams, 0, encMtrSen, 0, motorParams.length);
    System.arraycopy(encoderParams, 0, encMtrSen, motorParams.length, encoderParams.length);
		System.arraycopy(sensorParams, 0, encMtrSen, motorParams.length+encoderParams.length, sensorParams.length);
		
		deviceLink.transmit(communicationType, deviceID, encMtrSen, encoderParameters);	
	}
  ...
  */
	public void device_read_request(){
		...
		deviceLink.transmit(communicationType, deviceID, dummy, encoderRequest);
	}
  ...
  */
	public void device_write_torques(){
		...
		deviceLink.transmit(communicationType, deviceID, dummy, deviceTorques);
	}
